...
class OnlineWatcher {
  OnlineWatcher({this.onlineClasses, this.dataState})
  ...
  String get pathToChrome {
    String env(String variable) => Platform.environment[variable];
    final paths = [
  ...
  Future<void> _login(String user, String password) async {
    final loginButton = await page.$('#login');
  ...
  Stream<int> start() async* {
    assert(browser != null && page != null,
        'The class watcher must be initialized');
    assert(dataState is UserData);
    UserData state = dataState;
